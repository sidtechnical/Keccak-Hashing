// This is the same implementation from Previous Keccak permutation
...
unsigned long** pi_step(unsigned long **A)
{
	...
	for(j=0; j<5; j++){					//forall (x,y) in (0…4,0…4)
		for(k=0; k<5; k++)
		{
/*B[y,2*x+3*y] = rot(A[x,y], r[x,y])  -- Call to bit rotation function and Modulo 5 addition and multiplication as */
//per guidelines in Noppa 
